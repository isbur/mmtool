...
function test_wiki() {
	...
	error_count += CeL.test('CeL.wiki.parser', function(assert) {
		...
		assert([ 'A.svg', parser[0].name ], 'wiki.parse.file #1-2');
		assert([ 'b{{c|d[[e]]f}}', parser[0][4].toString() ], 'wiki.parse.file #1-3');
		wikitext = "[[File:a.svg|alt= alt_of_{{tl|t}} |  !!{{tl|t}}| 654px|thumb  ]]"; parser = CeL.wiki.parse(wikitext);
		assert([ wikitext, parser.toString() ], 'wiki.parse.file #2-1');
		assert([ '!!{{tl|t}}', parser.caption.toString() ], 'wiki.parse.file #2-2');
		assert([ '{{tl|t}}', parser.caption[1].toString() ], 'wiki.parse.file #2-3');
		assert([ '654px', parser.size ], 'wiki.parse.file #2-4');
		assert([ 'alt_of_{{tl|t}}', parser.alt.toString() ], 'wiki.parse.file #2-5');
		assert([ 'thumb', parser.format ], 'wiki.parse.file #2-6');
		wikitext = '[[Media:a.ogg|thumb |  a<br/>b   |200px | link= ABC]]'; parser = CeL.wiki.parse(wikitext);
		assert([ wikitext, parser.toString() ], 'wiki.parse.file #3-1');
		assert([ 'ABC', parser.link.toString() ], 'wiki.parse.file #3-2');
		assert([ 'thumb', parser.format ], 'wiki.parse.file #3-3');
		assert([ 'a<br/>b', parser.caption.toString() ], 'wiki.parse.file #3-4');
		...
		wikitext = '{{ {{tl|t}} | p }}'; parser = CeL.wiki.parse(wikitext);
		...
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[link#section]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[link#section|displayed]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[link#section|displayed|2]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[link#section|displayed|2#3]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[link#section#2|displayed|2#3]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[#section#2|displayed|2#3]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[text{abc}[[link]]text]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[file:abc.jpg]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[WP:abc]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[:file:abc.jpg]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[#]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[text{abc}text]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[#a| s[[r]] ]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[#a| s[[r] ]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		assert([ ' s[[r] ', CeL.wiki.parser(wikitext).parse()[1][2] ]);
		wikitext = 'a[[#a3| s[r] ]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[#a3| s[r]{3} ]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[#a3| s[r]-{3}- ]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a[[abc|s{{=}} -{ab}-]]b'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a [[b<!-- c -->]]d'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a [[:en:b<!-- c -->]] d'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a [[b{{=}}<!-- c -->]]d'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a [[:en:b{{=}}<!-- c -->]] d'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a [[<!-- c -->b{{=}}]]d'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a [[<!-- c -->:en:b{{=}}]]d'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a [[:<!-- c -->en:b{{=}}]] d'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a [[:en:<!-- c -->b{{=}}]] d'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a [[:en<!-- c -->:b{{=}}]] d'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = 'a \n==[[:en<!--comments-->:link{{=}}<!--comments-->]] ==\n d'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		...
		assert([ wikitext, parser.toString() ]);
		wikitext = '==t1==\n== t2 =='; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = '[[t#-{c}-]]'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = '[[t#a-{c}-b]]'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		wikitext = '==t==\n[[w:t/t#a-{c}-]] \n'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ]);
		...
		assert([ 'br', parser[1].tag ], 'wiki.parse: HTML single tag #2');
		wikitext = '1<br clear="all">2'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ 'tag_single', parser[1].type ], 'wiki.parse: HTML single tag #3');
		assert([ wikitext, parser.toString() ], 'wiki.parse: HTML single tag #4');
		assert([ ' clear="all"', parser[1][0].toString() ], 'wiki.parse: HTML single tag #5');

		wikitext = '1<b>a</b>2'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: HTML tag #1');
		assert([ 'b', parser[1].tag ], 'wiki.parse: HTML tag #2');
		wikitext = '1<b style="color:#000"  >{{a}}{{c}}</b>2'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: HTML tag #3');
		...
		assert([ '<pre class="c">\n==t==\nw\n</pre>', parser[1].toString() ], 'wiki.parse: HTML tag pre #2');
		assert([ ' class="c"', parser[1][0].toString() ], 'wiki.parse: HTML tag pre #3');
		assert([ '\n==t==\nw\n', parser[1][1].toString() ], 'wiki.parse: HTML tag pre #4');
		wikitext = '1<nowiki>\n==t==\nw\n</nowiki>2'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: nowiki #1');
		assert([ '<nowiki>\n==t==\nw\n</nowiki>', parser[1].toString() ], 'wiki.parse: nowiki #2');
		assert([ '', parser[1][0].toString() ], 'wiki.parse: nowiki #3');
		assert([ '\n==t==\nw\n', parser[1][1].toString() ], 'wiki.parse: nowiki #4');

		wikitext = '{{tl|1=<b a{{=}}"A">i</b>|p=P}}'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: {{=}} #1');
		wikitext = '{{tl|<b a{{=}}"A">i</b>}}'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: {{=}} #2');
		wikitext = '<b a{{=}}"A">i</b>'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: {{=}} #3');
		wikitext = '<b a="A">i</b>'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: {{=}} #4');
		...
		wikitext = '\nabc\n123\n'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ undefined, parser.each_section().sections[0].section_title ], 'wiki.parser.each_section #1-1: 沒有章節標題的文章');
		assert([ wikitext, parser.each_section().sections[0].toString() ], 'wiki.parser.each_section #1-2: 沒有章節標題的文章');
		assert([ wikitext, parser.each_section().sections.join('|') ], 'wiki.parser.each_section #1-3: 沒有章節標題的文章');
		wikitext = '==t1==\nabc\n123\n==t2==\n==t3=='; parser = CeL.wiki.parser(wikitext).parse();
		assert([ '\nabc\n123\n|\n|', parser.each_section().sections.join('|') ], 'wiki.parser.each_section #2-1: 以章節標題開頭的文章, 以章節標題結尾的文章, 章節標題+章節標題');
		assert([ '==t1==', parser.each_section().sections[0].section_title.toString() ], 'wiki.parser.each_section #2-2: 以章節標題開頭的文章, 以章節標題結尾的文章, 章節標題+章節標題');
		wikitext = 'cde\n==t1==\nabc\n123\n==t2==\n456'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ 'cde\n|\nabc\n123\n|\n456', parser.each_section().sections.join('|') ], 'wiki.parser.each_section #3-1: 正常文章');
		assert([ undefined, parser.each_section().sections[0].section_title ], 'wiki.parser.each_section #3-2: 正常文章');
		assert([ '==t1==', parser.each_section().sections[1].section_title.toString() ], 'wiki.parser.each_section #3-3: 正常文章');
		wikitext = 'aaa<ref>a[[b]]c</ref>bbb<ref name="r2">111</ref>ccc<ref name="r2" />'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parser.ref #1');
		assert([ 'r2', parser[3].attributes.name ], 'wiki.parser.ref #2: .name');
		...
		wikitext = '#1\n#2\nf'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: list #1');
		assert([ '#1\n#2', parser[0].toString() ], 'wiki.parse: list #1-1');
		assert([ '#', parser[0].list_type ], 'wiki.parse: list #1-1');
		assert([ 2, parser[0].length ], 'wiki.parse: list #1-2');
		wikitext = 'a\n*1\n*2\n*3\nf'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: list #2');
		assert([ '\n', parser[1] ], 'wiki.parse: list #2-1');
		assert([ '*1\n*2\n*3', parser[2].toString() ], 'wiki.parse: list #2-2');
		assert([ '*', parser[2].list_type ], 'wiki.parse: list #2-3');
		assert([ 3, parser[2].length ], 'wiki.parse: list #2-4');
		assert([ '2', parser[2][1] ], 'wiki.parse: list #2-5');
		wikitext = 'a\n#1\n#2\n##31\n##32\n#4\nf'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: list #3');
		wikitext = 'a{{T|\n*1\n*2\n}}f'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: list #4');
		wikitext = 'a{{T|*1\n*2\n}}f'; parser = CeL.wiki.parser(wikitext).parse();
		assert([ wikitext, parser.toString() ], 'wiki.parse: list #5');
		wikitext = 'a\n;t1:d1\n;t2:d2\n:d2-1\n;#1\n;#2\n;#3\n;t3\n:d3\nf'; parser = CeL.wiki.parser(wikitext).parse();
