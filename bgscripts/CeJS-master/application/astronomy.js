...
if (typeof CeL === 'function')
	CeL.run({
		...
		code : function(library_namespace) {
			...
			//
			SOLAR_RADIUS_RADIANS = SOLAR_RADIUS_ARCSECONDS
					* ARCSECONDS_TO_RADIANS,
			/**
			 * Geocentric semi-diameter of the Sun (in m) 日面的平均地心視半徑。
			 * 
			 * Math.atan(696342000 / 149597870700) / (2 * Math.PI) * 360 * 60 ≈
			 * 16′
			 * 
			 * @type {Number}
			 */
			...
			(function() {
				...
				LATITUDE_PATTERN = new RegExp(d.source + ms.source + '([NS])',
						'i');
				LONGITUDE_PATTERN = new RegExp(d.source + ms.source + '([EW])',
						'i');
			})();
			 ...
			 */
			function time_to_radians(hours, minutes, seconds, to_days,
					is_degrees) {
				if (seconds)
			 ...
			 * @returns {Number}radians
			 */
			function degrees_to_radians(degrees, arcminutes, arcseconds,
					to_days) {
				if (degrees < 0) {
					if (arcminutes)
				...
				return time_to_radians(degrees, arcminutes, arcseconds,
						to_days, true);
			}
			...
			/**
			 * 計算角度差距(減法)。 return (base - target), target 會先趨近於 base。或是說結果會向 0
			 * 趨近。
			 * 
			 ...
			 * 
			 * e.g., Geocentric semi-diameter of the Sun, apparent radius,
			 * 日面的地心視半徑。
			 * 
			 ...
			 *            object radius (in arcseconds) or object name. default:
			 *            sun
			 * @param {Number}[latitude]
			 *            object-centric latitude of the Earth.<br />
			 *            e.g., Saturnicentric latitude of the Earth (see
			 *            Chapter 45).<br />
			...
			function semidiameter(coordinates, radius, latitude) {
				...
				var distance = typeof coordinates === 'number'
						&& coordinates > 0 ? coordinates : coordinates.Δ
				...
				if (object === MOON_NAME) {
					if (false && coordinates.Δ > 0) {
						...
						if (latitude)
							// → 以觀測者為中心的座標中看到的月亮視半徑
							radius += radius
									* Math.sin(latitude * DEGREES_TO_RADIANS)
					 ...
					 * Third impression 1974.<br />
					 * p. 213.<br />
					 * As from 1963, the value of k is taken as 0.2724807, which
					 * leads to the same value of the semi-diameter as in the
					 * lunar ephemeris. The value 0.272274 for k is retained,
					 * however, in the calculation of duration on the central
					 * line of total solar eclipses, by way of applying an
					 * approximate correction for the irregularities of the
					 * lunar limb.
					 * 
					 * Explanatory Supplement to the Astronomical Almanac.<br />
					 * p. 425.<br />
					 * The lAU adopted a new value of k (k = 0.2725076) in
					 * August 1982.
					 */
					var k = 0.272481, sin_π = 6378.14 / distance;
					if (coordinates && coordinates.LHA) {
						// p. 280. formula 40.7
						var A = Math.cos(coordinates.δ)
								* Math.sin(coordinates.LHA),
						//
						B = Math.cos(coordinates.δ) * Math.cos(coordinates.LHA)
								- coordinates.ρcos_φp * sin_π,
						//
						C = Math.sin(coordinates.δ) - coordinates.ρsin_φp
								* sin_π;
						/**
						 * the topocentric distance of the Moon (that is, the
						 * distance from the observer to the center of the Moon)
						 * is Δ′=q*Δ, q being given by formula (40.7).
						 */
			 ...
			 *            longitude (°), time zone (e.g., UTC+8: 8), elevation
			 *            or geometric height (m) ]<br />
			 ...
			 * 此函數為為了舊曆元所做的修正。
			 * 
			 * longitude λ must be converted to the ephemeris longitude λ* by
			 * increasing it by 1.002738 ΔT, the sidereal equivalent of ΔT.
			 * 
			 ...
			 * p. 241.
			 * 
			 * The ephemeris meridian is 1.002 738 ΔT east of the Greenwich
			 * meridian, where ΔT=TT−UT1.
			 * 
			 ...
			 *            longitude α , latitude δ } in radians
			 ...
			 */
			function angular_distance(coordinates_1, coordinates_2,
					no_minor_squre) {
				...
				if (!no_minor_squre && Δα < .001
						&& (Δδ = Math.abs(δ1 - δ2)) < .001) {
					// 角度差接近於0或180度時，求取近似值。
					Δα *= Math.cos((δ1 + δ2) / 2);
			...
			function ecliptical_to_equatorial(coordinates, TT_JD, options) {
				...
				coordinates.α = Math.atan2(sin_λ * cos_ε - Math.tan(β) * sin_ε,
						Math.cos(λ));
				// geocentric declination 地心赤緯。
				coordinates.δ = Math.asin(Math.sin(β) * cos_ε + Math.cos(β)
						* sin_ε * sin_λ);
			...
			function equatorial_to_horizontal(coordinates, TT_JD, local) {
				...
				var α = coordinates.α,
				/**
				 * Phi: lower-case letter φ (or often its variant, ϕ)
				 * 觀測者緯度（北半球為正,南半球為負）
				 * 
				 * @type {Number}
				 */
				...
				coordinates.Alt = refraction(Math.asin(sin_φ * Math.sin(δ)
			...
			function get_horizontal(coordinates, TT_JD, options) {
				...
				if (options.elongation) {
					...
					// 月心座標下的地球距角i
					coordinates.i = Math.atan2(solar.Δ
							* Math.sin(coordinates.Ψ), coordinates.Δ - solar.Δ
							* cos_Ψ);
					// ψ和i在均在0到180度之間。

					// 月亮明亮邊緣的位置角
					coordinates.χ = Math.atan2(Math.cos(solar.δ)
							* Math.sin(solar.α - coordinates.α), Math
							.sin(solar.δ)
							* Math.cos(coordinates.δ)
							- Math.cos(solar.δ)
			...
			function dynamical_to_FK5(coordinates, τ) {
				...
				var _L = polynomial_value([ coordinates.L,
				...
				ΔL = 0.03916 * ARCSECONDS_TO_RADIANS * (cos_L + sin_L)
						* Math.tan(coordinates.B) - 0.09033
						* ARCSECONDS_TO_RADIANS;
				if (library_namespace.is_debug(3))
			 ...
			 *            <br />
			 *            {Boolean}options.FK5: translate VSOP87 coordinates to
			...
			function object_coordinates(TT_JD, object, options) {
				// 前置處理。
				if (!options && library_namespace.is_Object(object)
						&& object.object)
			...
			function JD_of_solar_angle(year, degrees) {
				...
				if (degrees % EQUINOX_SOLSTICE_DEGREES > 0)
					TT_JD += ((index === 3 ? equinox(year + 1, 0) : equinox(
							year, index + 1)) - TT_JD)
							// 以內插法(線性插值)取得近似值。
			...
			function lunar_coordinates(TT_JD, options) {
				...
				if (('V' in coordinates) || ('U' in coordinates)) {
					...
					if ('V' in coordinates) {
						...
						/**
						 * 修正地球章動 nutation: LEA-406 基於 LE406，未包含 nutations,
						 * librations。
						 * 
			...
			eclipse_coefficients_M = [ 2.5534, 29.10535670 * 1236.85,
					-0.0000014, -0.00000011 ].map(degrees_to_radians),
			// Moon's mean anomaly: (Mʹ, M′) 月亮的平近點角： in radians
			eclipse_coefficients_Mm = [ 201.5643, 385.81693528 * 1236.85,
					0.0107582, 0.00001238, -0.000000058 ]
					.map(degrees_to_radians),
			// Moon's argument of latitude: 月亮的緯度參數： in radians
			eclipse_coefficients_F = [ 160.7108, 390.67050284 * 1236.85,
					-0.0016118, -0.00000227, 0.000000011 ]
					.map(degrees_to_radians),
			// Longitude of the ascending node of the lunar orbit:
			// 月亮軌道升交點經度： in radians
			eclipse_coefficients_Ω = [ 124.7746, -1.56375588 * 1236.85,
					0.0020672, 0.00000215 ].map(degrees_to_radians),
			// p. 338. (47.6)
			...
			function mean_lunar_phase(year_month, phase, options) {
				...
				var
				// Moon's argument of latitude: 月亮的緯度參數： in radians
				F = normalize_radians(polynomial_value(eclipse_coefficients_F,
						T), true),
				//
				 ...
				 * 13.9 degrees, then there is certainly an eclipse; if the
				 * difference is larger than 21°.0, there is no eclipse; between
				 * these two values, the eclipse is uncertain at this stage and
				 * the case must be examined further.
				 */
				...
				* DEGREES_TO_RADIANS;
				...
				// p. 380.
				+ 0.0161 * Math.sin(2 * Mm) - 0.0097 * Math.sin(2 * F1)
						+ 0.0073 * E * Math.sin(Mm - M) - 0.0050 * E
						* Math.sin(Mm + M) - 0.0023 * Math.sin(Mm - 2 * F1)
						+ 0.0021 * E * Math.sin(2 * M) + 0.0012
						* Math.sin(Mm + 2 * F1) + 0.0006 * E
						* Math.sin(2 * Mm + M) - 0.0004 * Math.sin(3 * Mm)
						- 0.0003 * E * Math.sin(M + 2 * F1) + 0.0003
						* Math.sin(A1) - 0.0002 * E * Math.sin(M - 2 * F1)
						- 0.0002 * E * Math.sin(2 * Mm - M) - 0.0002
						* Math.sin(Ω);

				// p. 381. 高精度的日月位置計算需要幾百個項才能得到。
				var P = 0.2070 * E * Math.sin(M) + 0.0024 * E * Math.sin(2 * M)
						- 0.0392 * Math.sin(Mm) + 0.0116 * Math.sin(2 * Mm)
						- 0.0073 * E * Math.sin(Mm + M) + 0.0067 * E
						* Math.sin(Mm - M) + 0.0118 * Math.sin(2 * F1),
				// p. 381. 高精度的日月位置計算需要幾百個項才能得到。
				Q = 5.2207 - 0.0048 * E * Math.cos(M) + 0.0020 * E
						* Math.cos(2 * M) - 0.3299 * Math.cos(Mm) - 0.0060 * E
						* Math.cos(Mm + M) + 0.0041 * E * Math.cos(Mm - M),
				// p. 381.
				...
				// p. 381.
				u = 0.0059 + 0.0046 * E * Math.cos(M) - 0.0182 * Math.cos(Mm)
						+ 0.0004 * Math.cos(2 * Mm) + -0.0005
						* Math.cos(M + Mm),
				//
				abs_γ = Math.abs(γ),
				// 計算日月食的特性值。
				feature = {
					 ...
					 * 
					 * If F is near 0° or 360°, the eclipse occurs near the
					 * Moon's ascending node. If F is near 180°, the eclipse
					 * takes place near the descending node of the Moon's orbit.
					 */
				...
				if (phase === 0) {
					...
					if (abs_γ < 0.9972) {
						 ...
						 * 
						 * The quantity u denotes the radius of the Moon's
						 * umbral cone in the fundamental plane, again in units
						 * of the Earth's equatorial radius. The radius of the
						 * penumbral cone in the fundamental plane is u +
						 * 0.5461.
						 */
						// 影錐半徑
						...
						// p. 382. (54.2)
						feature.magnitude = (1.5433 + u - abs_γ)
								/ (0.5461 + 2 * u);
					} else {
						// 在地表上看不到日食。
			...
			/**
			 * the ellipticity of the Earth's spheroid. e^2 = 0.00669454 for
			 * 1968 onwards<br />
			 * 
			 * Reference 資料來源/資料依據:<br />
			 * Explanatory Supplement to the Astronomical Ephemeris. Third
			...
			function Besselian_elements(TT_JD, local, options) {
				...
				// p. 215.
				/**
				 * β☾: the latitude of the Moon at the time of conjunction in
				 * longitude
				 */
				...
				// p. 216.
				var b = Math.sin(sun_coordinates.π)
						/ Math.sin(moon_coordinates.π),
				// cos(α☉)
				...
				/**
				 * a and d designate the right ascension and declination of the
				 * point Z on the celestial sphere towards which the axis of the
				 * shadow is directed
				 */
				/** g*cos(d)*cos(a) */
				g_cos_d_cos_a = cos_δs * cos_αs - b * cos_δm * cos_αm,
				/** g*cos(d)*sin(a) */
				g_cos_d_sin_a = cos_δs * Math.sin(sun_coordinates.α) - b
						* cos_δm * Math.sin(moon_coordinates.α),
				/** g*cos(d) */
				g_cos_d = Math.sqrt(g_cos_d_cos_a * g_cos_d_cos_a
						+ g_cos_d_sin_a * g_cos_d_sin_a),
				/** g*sin(d) */
				...
				/**
				 * The angles f1, f2 which the generators of the penumbral
				 * (subscript 1) and umbral (subscript 2)<br />
				 * For 1968 onwards:
				 */
				...
				/**
				 * p. 218.<br />
				 * distances of the vertices of the penumbral and umbral cones
				 * above the fundamental plane are thus, in units of the
				...
				/**
				 * l1,l2: the radii of the penumbra and umbra on the fundamental
				 * plane
				 */
				l1 = c1 * tan_f1,
				// p. 239.
				...
				var elements_1_hour = Besselian_elements(TT_JD + 1
						/ ONE_DAY_HOURS, local, {
					get_diff : true
				});
				...
				// p. 220.
				/**
				 * It is always sufficient to calculate them for the integral
				 * hour nearest conjunction
				 */
				...
				if (tmp >= 0) {
					...
					// p. 223.
					central.θ = Math.atan(x, -central.η1 * sin_d1 + central.ζ1
							* cos_d1);
					central.λ = μ - central.θ;
					central.longitude = from_ephemeris_longitude(central.λ,
							TT_JD);
					// sin(φ1)
					...
					//
					/ Math.sqrt(1 - tmp * tmp)
							/ Math.sqrt(1 - eccentricity_square), true),
							'decimal');
				} else {
					// 超過了地球的半徑，無日月蝕。
					central.ζ2 = tmp;
				...
				/**
				 * Q: the position angle (measured eastwards from the north,
				 * i.e. from the y-axis towards the x-axis) of the axis from the
				 * observer
				 */

				var
				/**
				 * At a height ζ above the fundamental plane the radius L of the
				 * shadow is given by: L = l - ζ * tan(f)
				 */
				// penumbra
				L1 = l1 - ζ * tan_f1,
				// umbra
				...
				/**
				 * semi-duration s of the total, or annular, phase on the
				 * central line
				 */
				s = Math.abs(L2) / n,
				...
				function get_limit(southern, penumbra) {
					...
					/**
					 * p. 225.<br />
					 * If the latter is not available, as for example in the
					 * case of non-central eclipses, it is sufficient to set: _ζ =
					 * 0
					 */
					var _ζ = ζ || 0, cos_Q = 1;
					var _ξ, last_ζ;
					do {
						...
						// 斜邊
						var hypotenuse = Math.sqrt(adjacent * adjacent
								+ opposite * opposite);
						// var Q = Math.atan2(bp, (cp - _ζ * μp * cos_d));

						cos_Q = opposite / hypotenuse;
						/**
						 * The sign of cos Q is positive for the northern limit
						 * of a total eclipse and the southern limit of an
						 * annular eclipse; it is negative for the southern
						 * limit of a total eclipse and the northern limit of an
						 * annular eclipse.
						 */
					...
					return isNaN(φ) ? null : [
							...
							format_radians(normalize_radians(φ, true),
									'decimal') ];
				}
				...
				/**
				 * If the angle (Q - M) is imaginary, there are no end points to
				 * the curve, and the angle Q takes all values from 0° to 360°.
				 */
				var outline = [
			...
			Besselian_elements.lunar = function(TT_JD, options) {
				...
				sm = semidiameter(moon_coordinates, MOON_NAME)
						* ARCSECONDS_TO_RADIANS,
				// penumbral radius
				f1 = tmp + ss,
				// umbral radius
				...
				/**
				 * angular distance between the centres of the Moon and the
				 * shadow
				 */
				var m = Math.sqrt(x * x + y * y);
				...
				return {
					...
					/**
					 * The latitudes (φ) and ephemeris longitudes (λ*) of the
					 * places that have the Moon in the zenith at given times
					 */
					latitude : format_radians(normalize_radians(
							moon_coordinates.δ, true), 'decimal'),
					// ephemeris sidereal time - α☾
					longitude : from_ephemeris_longitude(GAST(
							TT_of(TT_JD, true), TT_JD)
							- moon_coordinates.α, TT_JD)
				};
			};
			...
			function eclipse_JD(TT_JD, phase, options) {
				...
				if (eclipse_data.type) {
					eclipse_data.name = eclipse_data.type + ' '
							+ eclipse_data.object + ' eclipse';
				...
				var βm = TT.l.π - TT.s.π + (moon_radius - sun_radius)
						* ARCSECONDS_TO_RADIANS;
				if (βm > phase === 0 ? solar_eclipse_limit
						: lunar_eclipse_limit)
					return;
				 ...
				 * 月面的地心視半徑 (度)。
				 * 
				 * TODO: use (coordinates.Δ * AU_TO_METERS) instead of
				 * LUNAR_DISTANCE_M
				 * 
				 * @type {Number}
				 */
				...
				* TERRA_RADIUS_M / LUNAR_DISTANCE_M)
				...
				if (phase === 2) {
					...
					/**
					 * Equatorial horizontal parallax of the Sun, and 1.01 ≅ 1 +
					 * 1/85 - 1/594. 太陽赤道地平視差 (度)
					 * 
					 * Astronomical Almanac 2011:<br />
			...
			(year_month, phase, options) {
				...
				TT_JD = library_namespace.find_root(
						...
						// / 12: 月日視黃經差每日必於 12°–13°之內。
						// 因此每度耗時必小於 1/12 日。此處取最大值。
						TT_JD - (result_degrees - degrees) / 12, TT_JD,
						degrees, {
							y1 : result_degrees
						});
			 ...
			 * According to HORIZONS (DE-431), it's about 9/28 19:54 (月亮追過太陽) or
			 * 9/29 00:08 (視角度差最小).
			 * 
			...
			function lunar_phase(year, phase, options) {
				...
				// options.duration = 2
				if (!options.duration) {
					UT_JD = UT_of((operator || accurate_lunar_phase)(year,
							phase, options));
					return UT_JD;
				}
				...
				//
					...
					if (phase_data = phase_data[phase])
						...
						+ mean_lunar_phase_days
								/ (DAYS_OF_JULIAN_CENTURY / 100)) {
							UT_JD = UT_of((operator || mean_lunar_phase)(
									year_month, phase, options));
							date = library_namespace.JD_to_Date(UT_JD);
							if (!operator
							// auto: 在特別可能有問題的時候採用精準值。
							&& ((hours = date.getHours()) < 2 || hours > 21)) {
								UT_JD = UT_of(accurate_lunar_phase(year_month,
										phase, options));
								date = library_namespace.JD_to_Date(UT_JD);
							}
			...
			function lunar_phase_of_JD(UT_JD, options) {
				...
				if (phase !== _phase
						...
						&& phase !== (_phase = Math
								.floor(lunar_phase_angle_of_JD(TT_JD) / 90))) {
			...
			function midnight_of(UT_JD, minute_offset, get_local_Date) {
				...
				var day_offset = (minute_offset | 0) / (ONE_DAY_HOURS * 60)
						- .5;
				...
				if (get_local_Date) {
					...
					//
					UT_JD + (minute_offset - default_offset)
							/ (60 * ONE_DAY_HOURS));
					// 歸零用。
			 ...
			 *            e.g., UTC+8: 8 * 60 = 480
			 * 
			 * @returns {Array} 年朔日 = [ [ Julian date (JD of 日常生活時間 UT), JD,
			 *          ...], 冬至所在月 ]
			 */
			 ...
			 *            UT JDN
			 * @param {Number}type
			 *            0:lower culmination, 1:rise, 2:upper culmination,
			 *            3:set
			 * @param {String}object
			 *            天體 (e.g., planets 行星).
			 * @param {Boolean}force
			...
			function rise_set(local, JDN, type, object, force) {
				...
				function angle(TT_JD) {
					...
					var angle = type % 2 === 1 || type > 3 ? coordinates.Alt
							+ Δ
					// 上中天: LHA: 2π → 應為0
					// 下中天: LHA: 應為π
				...
				if (false)
					for (var m = 0; m < 4 * 60; m++)
						angle(TT_of(UT_JD + hour0 / ONE_DAY_HOURS + m
								/ ONE_DAY_HOURS / 60));

				UT_JD = UT_of(library_namespace.find_root(angle, TT_of(UT_JD
						+ hour0 / ONE_DAY_HOURS), TT_of(UT_JD + hour1
						/ ONE_DAY_HOURS), 0,
			...
			function Coordinates(object, date, local, options) {
				...
				// 紀錄時間。
				this.JD = Number.isFinite(date) ? date
						: library_namespace.Julian_day.JD(date);
			 ...
			 *            get apogee 遠地點
			 * 
			 * @returns {Array}[ TT in JD, 距離 in km, Moon's Equatorial
			...
			function lunar_perigee_apogee(year_month, apogee) {
				...
				parallax = lunar_perigee_apogee_Δ(
						apogee ? apogee_coefficients_parallax
			...
			// T 時刻的月亮平距角：
			lunar_perigee_apogee_D = [ 171.9179, 335.9106046 * 1325.55,
					0.0100383, -0.00001156, 0.000000055 ]
					.map(degrees_to_radians),
			// Sun's mean anomaly:
			// 太陽平近點角：
			lunar_perigee_apogee_M = [ 347.3477, 27.1577721 * 1325.55,
					-0.0008130, -0.0008130 ].map(degrees_to_radians),
			// Moon's argument of latitude:
			// 月亮緯度參數：
			lunar_perigee_apogee_F = [ 316.6109, +364.5287911 * 1325.55,
					-0.0125053, -0.0000148 ].map(degrees_to_radians),
			/**
			...
			// 近地點 JD 修正量係數。
			[ 2, , , -1.6769 ], [ 4, , , .4589 ], [ 6, , , -.1856 ],
					[ 8, , , .0883 ], [ 2, -1, , -.0773, .00019 ],
					[ , 1, , .0502, -.00013 ], [ 10, , , -.0460 ],
					[ 4, -1, , .0422, -.00011 ], [ 6, -1, , -.0256 ],
					[ 12, , , .0253 ], [ 1, , , .0237 ], [ 8, -1, , .0162 ],
					[ 14, , , -.0145 ], [ , , 2, .0129 ], [ 3, , , -.0112 ],
					[ 10, -1, , -.0104 ], [ 16, , , .0086 ],
					[ 12, -1, , .0069 ], [ 5, , , .0066 ], [ 2, , 2, -.0053 ],
					[ 18, , , -.0052 ], [ 14, -1, , -.0046 ],
					[ 7, , , -.0041 ], [ 2, 1, , .0040 ], [ 20, , , .0032 ],
					[ 1, 1, , -.0032 ], [ 16, -1, , .0031 ],
					[ 4, 1, , -.0029 ], [ 9, , , .0027 ], [ 4, , 2, .0027 ],
					[ 2, -2, , -.0027 ], [ 4, -2, , .0024 ],
					[ 6, -2, , -.0021 ], [ 22, , , -.0021 ],
					[ 18, -1, , -.0021 ], [ 6, 1, , .0019 ],
					[ 11, , , -.0018 ], [ 8, 1, , -.0014 ],
					...
					[ 3, 2, , .0011 ], [ 4, -2, 2, -.0011 ],
					[ 1, 2, , -.0010 ], [ 22, -1, , -.0009 ],
					[ , , 4, -.0008 ], [ 6, , -2, .0008 ], [ 2, 1, -2, .0008 ],
					[ , 2, , .0007 ], [ , -1, 2, .0007 ], [ 2, , 4, .0007 ],
					[ , -2, 2, -.0006 ], [ 2, 2, -2, -.0006 ],
			...
			[ , , , 3629.215 ], [ 2, , , 63.224 ], [ 4, , , -6.990 ],
					[ 2, -1, , 2.834, -.0071 ], [ 6, , , 1.927 ],
					[ 1, , , -1.263 ], [ 8, , , -.702 ],
					[ , 1, , .696, -.0017 ], [ , , 2, -.690 ],
					[ 4, -1, , -.629, .0016 ], [ 2, , -2, -.392 ],
					[ 10, , , .297 ], [ 6, -1, , .260 ], [ 3, , , .201 ],
					[ 2, 1, , -.161 ], [ 1, 1, , .157 ], [ 12, , , -.138 ],
					[ 8, -1, , -.127 ], [ 2, , 2, .104 ], [ 2, -2, , .104 ],
					[ 5, , , -.079 ], [ 14, , , .068 ], [ 10, -1, , .067 ],
					[ 4, 1, , .054 ], [ 12, -1, , -.038 ], [ 4, -2, , -.038 ],
					[ 7, , , .037 ], [ 4, , 2, -.037 ], [ 16, , , -.035 ],
					[ 3, 1, , -.030 ], [ 1, -1, , .029 ], [ 6, 1, , -.025 ],
					[ , 2, , .023 ], [ 14, -1, , .023 ], [ 2, 2, , -.023 ],
					[ 6, -2, , .022 ], [ 2, -1, -2, -.021 ], [ 9, , , -.020 ],
					[ 18, , , .019 ], [ 6, , 2, .017 ], [ , -1, 2, .014 ],
					[ 16, -1, , -.014 ], [ 4, , -2, .013 ], [ 8, 1, , .012 ],
					[ 11, , , .011 ], [ 5, 1, , .010 ], [ 20, , , -.010 ] ],
			//
			apogee_coefficients_JD = [
			// 遠地點 JD 修正量係數。
			[ 2, , , .4392 ], [ 4, , , .0684 ], [ , 1, , .0456, -.00011 ],
					[ 2, -1, , .0426, -.00011 ], [ , , 2, .0212 ],
					[ 1, , , -.0189 ], [ 6, , , .0144 ], [ 4, -1, , .0113 ],
					[ 2, , 2, .0047 ], [ 1, 1, , .0036 ], [ 8, , , .0035 ],
					[ 6, -1, , .0034 ], [ 2, , -2, -.0034 ],
					[ 2, -2, , .0022 ], [ 3, , , -.0017 ], [ 4, , 2, .0013 ],
					[ 8, -1, , .0011 ], [ 4, -2, , .0010 ], [ 10, , , .0009 ],
					[ 3, 1, , .0007 ], [ , 2, , .0006 ], [ 2, 1, , .0005 ],
					[ 2, 2, , .0005 ], [ 6, , 2, .0004 ], [ 6, -2, , .0004 ],
					[ 10, -1, , .0004 ], [ 5, , , -.0004 ],
			...
			// 遠地點赤道地平視差修正量係數。
			[ , , , 3245.251 ], [ 2, , , -9.147 ], [ 1, , , -.841 ],
					[ , , 2, .697 ], [ , 1, , -.656, .0016 ], [ 4, , , .355 ],
					[ 2, -1, , .159 ], [ 1, 1, , .127 ], [ 4, -1, , .065 ],
					[ 6, , , .052 ], [ 2, 1, , .043 ], [ 2, , 2, .031 ],
					[ 2, , -2, -.023 ], [ 2, -2, , .022 ], [ 2, 2, , .019 ],
					[ , 2, , -.016 ], [ 6, -1, , .014 ], [ 8, , , .010 ] ];
