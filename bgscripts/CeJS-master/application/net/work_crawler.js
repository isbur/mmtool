...
function module_code(library_namespace) {
	...
	function extract_work_id_from_URL(work_information) {
		if (typeof work_information === 'string'
			...
			if (!PATTERN_work_id_of_url) {
				PATTERN_work_id_of_url = this
						.full_URL(this.work_URL, 'work_id');
				if (PATTERN_work_id_of_url.endsWith('work_id/')) {
					// 允許不用"/"結尾。
					PATTERN_work_id_of_url = PATTERN_work_id_of_url
							.slice(0, -1);
				}
	...
	function get_work(work_title, callback) {
		...
		function finish_up(work_data) {
			if (work_data && work_data.title) {
				...
				set_work_status(work_data, gettext('找到%1個作品：%2',
	...
	function check_downloaded_chapter_url(work_data, new_chapter_list,
		...
		&& Array.isArray(work_data.chapter_list)) {
			...
			if (chapter_list_to_check.length === 0) {
				library_namespace.log('check_downloaded_chapter_url: 跳過所有章節');
			} else {
				library_namespace.log('check_downloaded_chapter_url: 僅檢查: '
						+ chapter_list_to_check.join(', '));
			}
	...
	function get_work_data(work_id, callback, error_count) {
		...
		function process_chapter_list_data(html) {
			...
			if (typeof _this.get_chapter_list === 'function') {
				try {
					...
					library_namespace.error([ _this.id + ': ', {
						T : '.get_chapter_list() throw error.'
					} ]);
			...
			&& work_data.last_download.chapter === work_data.chapter_count) {
				// 跳過本作品不處理。
				library_namespace.log('process_chapter_list_data: Skip '
						+ work_data.id
	...
	function pre_get_chapter_data(work_data, chapter_NO, callback) {
		...
		var chapter_data = Array.isArray(work_data.chapter_list)
				&& work_data.chapter_list[chapter_NO - 1], skip_this_chapter = chapter_data
				&& (chapter_data.skip_this_chapter === true ? '跳過本章節不下載。'
						: chapter_data.skip_this_chapter);
		...
		if (!skip_this_chapter && this.chapter_filter) {
			...
			|| !chapter_data.part_title.toLowerCase().includes(chapter_filter))) {
				skip_this_chapter = '不在 chapter_filter 所篩範圍內。跳過本章節不下載。';
			}
		}

		if (skip_this_chapter) {
			library_namespace.debug('pre_get_chapter_data: Skip '
					+ (chapter_data.part_title ? '[' + chapter_data.part_title
							+ '] ' : '') + '[' + chapter_data.title + ']: '
					+ skip_this_chapter);
		...
		var next = chapter_time_interval > 0 ? (function() {
			...
			//
			gettext('下載第%1章之章節資訊前先等待 %2。', chapter_NO,
			//
	...
	function set_chapter_NO_via_title(chapter_data, default_NO, default_unit) {
		...
		library_namespace.warn('Can not determine chapter NO from '
				...
				try {
					...
					if (chapter_data === _this.REGET_PAGE) {
						...
						? chapter_time_interval > 0 ? '等待 %2 之後再重新取得作品資訊頁面： %1'
								: '重新取得作品資訊頁面： %1'
						//
						: chapter_time_interval > 0 ? '等待 %2 之後再取得作品資訊頁面： %1'
								: '取得作品資訊頁面： %1';
